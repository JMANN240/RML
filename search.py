def search(cur, term):
	if term is None:
		return []
	res = cur.execute('SELECT * FROM recipes WHERE name LIKE ?', (f"%{term}%",))
	...
	name_match_ids = [match['id'] for match in name_matched_recipes]
	...
	res = cur.execute(f"SELECT * FROM recipes WHERE id IN ({','.join('?'*len(ingredient_match_ids))})", ingredient_match_ids)
	ingredient_matched_recipes = res.fetchall()

	res = cur.execute(f"SELECT * FROM recipes WHERE id IN ({','.join('?'*len(step_match_ids))})", step_match_ids)
